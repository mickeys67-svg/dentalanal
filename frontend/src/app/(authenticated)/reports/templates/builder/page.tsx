            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">원하는 위젯을 조합하여 브랜드만의 리포트 형식을 만드세요.</p>
                    </div>
                </div>
                <button
                    onClick={handleSave}
                    disabled={isPending}
                    className="flex items-center gap-2 bg-primary text-white px-6 py-2.5 rounded-xl font-bold shadow-lg hover:bg-opacity-90 transition-all disabled:opacity-60"
                >
            ...
            <div className="grid grid-cols-12 gap-8">
                ...
                <div className="col-span-12 lg:col-span-4 space-y-6">
                        ...
                        <div className="space-y-3">
                            {AVAILABLE_WIDGETS.map((w) => (
                                <button
                                    key={w.type}
                                    onClick={() => addWidget(w)}
                                    className="w-full flex items-start gap-4 p-4 rounded-2xl hover:bg-primary/10 hover:border-primary border border-transparent transition-all group text-left"
                                >
                                    <div className="p-3 bg-secondary text-muted-foreground group-hover:bg-white group-hover:text-primary rounded-xl transition-all">
                                        <w.icon className="w-5 h-5" />
                                    </div>
                                    <div>
                                        <h4 className="text-sm font-bold text-foreground group-hover:text-primary">{w.name}</h4>
                    ...
                    <div className="bg-primary/10 rounded-3xl p-6 border border-primary/30">
                        <h4 className="text-xs font-bold text-primary uppercase tracking-widest mb-4">템플릿 기본 정보</h4>
                        <div className="space-y-4">
                            <div>
                                <label className="text-[11px] font-bold text-muted-foreground ml-1">템플릿 이름</label>
                                <input
                                    type="text"
                                    value={name}
                                    onChange={(e) => { setName(e.target.value); setFormError(''); }}
                                    placeholder="예: 월간 성과 상세 분석"
                                    className="w-full mt-1 px-4 py-3 rounded-xl border-none ring-1 ring-border focus:ring-2 focus:ring-primary text-sm font-medium"
                                />
                            </div>
                            <div>
                                <label className="text-[11px] font-bold text-muted-foreground ml-1">설명 (선택)</label>
                                <textarea
                                    value={description}
                                    onChange={(e) => setDescription(e.target.value)}
                                    placeholder="템플릿에 대한 설명을 입력하세요."
                                    rows={3}
                                    className="w-full mt-1 px-4 py-3 rounded-xl border-none ring-1 ring-border focus:ring-2 focus:ring-primary text-sm font-medium resize-none"
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div className="col-span-12 lg:col-span-8 space-y-4">
                    <div className="flex items-center justify-between mb-2">
                        <h2 className="text-lg font-bold text-foreground">리포트 레이아웃 프리뷰</h2>
                        <span className="text-[10px] font-bold text-muted-foreground uppercase">총 {selectedWidgets.length}개 위젯 배치됨</span>
                    </div>

                    {selectedWidgets.length === 0 ? (
                        <div className="bg-background border-2 border-dashed border-border rounded-[2.5rem] py-32 flex flex-col items-center justify-center text-muted-foreground animate-pulse">
                        ...
                        <div className="space-y-4">
                            {selectedWidgets.map((sw, idx) => (
                                <div key={sw.id} className="bg-card rounded-3xl border border-border p-6 shadow-sm flex items-center gap-6 group hover:border-primary/30 transition-all">
                                    <div className="text-muted-foreground/60 group-hover:text-primary transition-colors cursor-grab active:cursor-grabbing">
                                        <GripVertical className="w-5 h-5" />
                                    </div>
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="text-[10px] font-black text-primary">0{idx + 1}</span>
                                            <h4 className="text-sm font-bold text-foreground">{sw.title}</h4>
                                        </div>
                                        <div className="h-12 bg-secondary rounded-xl flex items-center px-4">
                                            <span className="text-[10px] font-bold text-muted-foreground/60 uppercase letter tracking-tighter">Widget Type: {sw.type} Preview Area</span>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => removeWidget(sw.id)}
                                        className="p-3 rounded-2xl bg-secondary text-muted-foreground/60 hover:bg-red-50 hover:text-red-500 transition-all"
                                    >
