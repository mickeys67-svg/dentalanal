    ...
    return (
        <div className="space-y-10 p-8 animate-in fade-in duration-700 max-w-7xl mx-auto">
            <div className="flex items-end justify-between border-b border-border pb-8">
                <div>
                    <h1 className="text-4xl font-extrabold text-foreground tracking-tight mb-2">프로젝트 관리 허브</h1>
                    <p className="text-lg text-muted-foreground font-medium">분석 대상 정보를 관리하고 새로운 분석 프로젝트를 시작하세요.</p>
                </div>
                {viewMode === 'LIST' ? (
                    <button
                    ...
                    <button
                        ...
                        className="text-muted-foreground font-bold hover:text-foreground transition-all flex items-center gap-2"
                    >
                        목록으로 돌아가기
                    </button>
                )}
            ...
            {viewMode === 'CREATE' ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {clients.map(client => (
                        <div key={client.id} className="group bg-card border border-border rounded-3xl p-8 shadow-sm hover:shadow-xl hover:shadow-secondary/50 transition-all relative overflow-hidden">
                            <div className="absolute top-2 right-2 p-2 opacity-20 group-hover:opacity-100 transition-opacity z-10">
                                <button
                                    ...
                                    className="p-3 text-muted-foreground hover:text-red-500 hover:bg-red-50 rounded-full transition-all"
                                    title="프로젝트 삭제"
                                >
                            ...
                            <div className="mb-6">
                                <span className="inline-block px-3 py-1 bg-primary/10 text-primary text-[10px] font-bold rounded-full mb-3 uppercase tracking-wider">
                                    {client.industry || '기타'}
                                </span>
                                <h3 className="text-2xl font-bold text-foreground leading-tight">{client.name}</h3>
                            </div>
                            ...
                            {editingEmailId === client.id ? (
                                    <div className="flex items-center gap-2">
                                        <div className="flex-1 flex items-center gap-1 border border-primary rounded-lg px-2 py-1 bg-primary/10 focus-within:ring-2 focus-within:ring-primary">
                                            ...
                                            <input
                                                ...
                                                placeholder="이메일 입력"
                                                autoFocus
                                                className="flex-1 text-xs bg-transparent outline-none text-foreground placeholder-muted-foreground"
                                            />
                                        </div>
                                        <button
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors group/email"
                                    title="리포트 수신 이메일 편집"
                                >
                                    <Mail className="w-3 h-3" />
                                    <span className={(client as any).email ? 'text-muted-foreground font-medium' : 'italic'}>
                            ...
                            {editingConvId === client.id ? (
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => setEditingConvId(null)} className="p-1.5 text-muted-foreground hover:text-muted-foreground hover:bg-secondary rounded-lg transition-colors"><XIcon className="w-3 h-3" /></button>
                                </div>
                            ) : (
                                <button
                                    onClick={() => { setEditingConvId(client.id); setEditConvValue(String((client as any).conversion_value || '')); }}
                                    className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-emerald-600 transition-colors group/conv"
                                    title="전환당 수익 편집 (ROAS 계산 기준)"
                                >
                                    <DollarSign className="w-3 h-3" />
                                    <span className={(client as any).conversion_value ? 'text-muted-foreground font-medium' : 'italic'}>
                            ...
                            {editingFeeId === client.id ? (
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => setEditingFeeId(null)} className="p-1.5 text-muted-foreground hover:text-muted-foreground hover:bg-secondary rounded-lg transition-colors"><XIcon className="w-3 h-3" /></button>
                                </div>
                            ) : (
                                <button
                                    onClick={() => { setEditingFeeId(client.id); setEditFeeValue(String((client as any).fee_rate ? Math.round((client as any).fee_rate * 100) : '')); }}
                                    className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-amber-600 transition-colors group/fee"
                                    title="대행 수수료율 편집"
                                >
                                    <Percent className="w-3 h-3" />
                                    <span className={(client as any).fee_rate ? 'text-muted-foreground font-medium' : 'italic'}>
                            ...
                            <div className="flex items-center justify-between mt-4 pt-6 border-t border-border">
                                <div className="text-xs text-muted-foreground space-y-1">
                                    <div className="flex items-center gap-1">
                                        <span className="font-bold text-foreground">3개의 타겟</span> 연동됨
                                    </div>
                                    {client.created_at && (
                    ...
                    {clients.length === 0 && !isLoading && (
                        <div className="col-span-full py-20 text-center bg-background rounded-3xl border-2 border-dashed border-border">
                            ...
                            <p className="text-muted-foreground font-medium">아직 등록된 프로젝트가 없습니다.<br />첫 번째 분석을 시작해보세요!</p>
                            <button
                                onClick={() => setViewMode('CREATE')}
